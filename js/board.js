    ...
    <div draggable="true" id="todo${element['id']}" ondragstart="startDragging(${element['id']})" class="todo2">
        ...
        <div class="card-icons-container">
            ...
            <div class="card-icon-edit-container" id="card-icon-edit-container${element['id']}">
                <img onclick="open_dialog(${element['id']})" class="card-icon-edit" src="./img/edit.png">
            </div>
            <div id="card-icon-trash${element['id']}" class="card-icons-right">
                <img onclick="delete_todo(${element['id']})" src="./img/trash-2-32.png">
            </div>
        </div>
    </div>
        `;
...
function change_card_border_color() {
    for (i = 0; i < todos.length; i++) {
        if (todos[i].urgency == 'Low') {
            document.getElementById(`todo${i}`).style.border = "4px solid #0BF518";
        }
        if (todos[i].urgency == 'Medium') {
            document.getElementById(`todo${i}`).style.border = "4px solid #FFF205";
        }
        if (todos[i].urgency == 'High') {
            document.getElementById(`todo${i}`).style.border = "4px solid #F5004B";
        }
    }
}
